#!/usr/bin/env python3
"""Apertura 2025 J9 mechanical V2.1 selector + V2.2 fractional-Kelly shadows."""
from __future__ import annotations
import json
from pathlib import Path
ROOT=Path(__file__).resolve().parent
MODEL_OUT=ROOT/'ap25_j9_model_output.txt'
PRICE_SNAPSHOT=ROOT/'ap25_j9_price_snapshot_pre_cutoff.json'
BUDGET=100.0
CANONICAL_OPENING=520.5012159211471
SHADOW_OPENINGS={'FK15':524.9675765344799,'FK20':534.1361659359291,'FK25':543.4036686653837}
...
def load_model():
    text=MODEL_OUT.read_text(encoding='utf-8').strip(); prefix='AP25_J9_MODEL_ONLY '
    if not text.startswith(prefix): raise SystemExit('Unexpected J9 model output format')
    obj=json.loads(text[len(prefix):])
    if obj['training_count']!=1088: raise SystemExit('J9 training-count drift')
    if obj['jornada']!='J9': raise SystemExit('Wrong model output')
    return obj
...
def main():
    ...
    out={'experiment':'EXP-003','tournament':'Apertura 2025','jornada':'J9',
         'integrity_class':'MECHANICAL_REPLAY_RESEARCHER_J9_OUTCOME_EXPOSED_BEFORE_SELECTION_FREEZE','promotion_eligibility':False,
         ...
         'notes':['No J9 outcome is used in the calculations.','J9 is NO-PROMOTION because one J9 outcome was exposed during legitimate pre-J9 backfill verification.',
    ...
    print('AP25_J9_SELECTION '+json.dumps(out,ensure_ascii=False,sort_keys=True),flush=True)
