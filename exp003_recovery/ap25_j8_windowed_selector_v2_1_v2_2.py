#!/usr/bin/env python3
"""Apertura 2025 J8 mechanical V2.1 selector + V2.2 fractional-Kelly shadows."""
from __future__ import annotations
import json
from pathlib import Path
ROOT=Path(__file__).resolve().parent
MODEL_OUT=ROOT/'ap25_j8_model_output.txt'
PRICE_SNAPSHOT=ROOT/'ap25_j8_price_snapshot_pre_cutoff.json'
BUDGET=100.0
CANONICAL_OPENING=514.1012159211471
SHADOW_OPENINGS={'FK15':517.5832819052672,'FK20':524.1652345916573,'FK25':530.782654672085}
...
def load_model():
    text=MODEL_OUT.read_text(encoding='utf-8').strip(); prefix='AP25_J8_MODEL_ONLY '
    if not text.startswith(prefix): raise SystemExit('Unexpected J8 model output format')
    obj=json.loads(text[len(prefix):])
    if obj['training_count']!=1078: raise SystemExit('J8 training-count drift')
    if obj['jornada']!='J8': raise SystemExit('Wrong model output')
    return obj
...
def main():
    ...
    out={'experiment':'EXP-003','tournament':'Apertura 2025','jornada':'J8',
         'integrity_class':'MECHANICAL_REPLAY_RESEARCHER_FUTURE_TOURNAMENT_EXPOSURE_BEFORE_SELECTION_FREEZE','promotion_eligibility':False,
         ...
         'notes':['No J8 outcome is used in the calculations.','J8 is NO-PROMOTION due to future-tournament researcher exposure during schedule retrieval.',
    ...
    print('AP25_J8_SELECTION '+json.dumps(out,ensure_ascii=False,sort_keys=True),flush=True)
