#!/usr/bin/env python3
"""Apertura 2025 J5 model-only replay under frozen V0.1R + calibration shadow.

Researcher became J5 outcome-exposed during schedule verification, so J5 is
permanently NO-PROMOTION. No J5 result is an input. Training includes only
completed Apertura 2025 J1-J4 plus frozen historical seasons.
"""
...
MODEL_CUTOFF=datetime(2025,8,15,0,0,0)
CAL_K=0.5620833333
...
FIXTURES=[
("AP25J5-01","2025-08-15","Puebla FC","Atlético San Luis"),
("AP25J5-02","2025-08-15","Club Necaxa","Club León"),
("AP25J5-03","2025-08-16","Deportivo Guadalajara","FC Juárez"),
("AP25J5-04","2025-08-16","Deportivo Toluca","Pumas UNAM"),
("AP25J5-05","2025-08-16","CF Pachuca","Club Tijuana"),
("AP25J5-06","2025-08-16","UANL Tigres","CF América"),
("AP25J5-07","2025-08-16","Cruz Azul","Santos Laguna"),
("AP25J5-08","2025-08-17","Gallos Blancos","Atlas Guadalajara"),
("AP25J5-09","2025-08-17","CF Monterrey","Mazatlán FC")]
...
def main():
    ...
    tr=sorted(hist+J1_FROZEN+J2_FROZEN+J3_FROZEN+J4_FROZEN,key=lambda m:(m.date,m.home,m.away,m.hg,m.ag))
    if len(tr)!=1051: raise SystemExit(f"J5 training count drift: expected 1051, got {len(tr)}")
    if any(m.date>=MODEL_CUTOFF for m in tr): raise SystemExit("J5 leakage detected")
    ...
    out={"experiment":"EXP-003","tournament":"Apertura 2025","jornada":"J5","model":"V0.1R_DC_ENSEMBLE",
         "calibration_shadow":"CAL_SHRINK_GLOBAL_V0_1","model_cutoff":"2025-08-15T00:00:00-06:00","training_count":len(tr),
         "training_includes":"Apertura 2025 J1-J4 completed fixtures","integrity_class":"DETERMINISTIC_REPLAY_RESEARCHER_OUTCOME_EXPOSED",
         "promotion_eligibility":False,"fixtures":rows,"notes":["No J5 result is an input.","Raw V0.1R remains canonical for V2.1 selection.","J5 cannot promote changes because researcher saw J5 outcomes during schedule verification."]}
    print("AP25_J5_MODEL_ONLY",json.dumps(out,ensure_ascii=False,sort_keys=True),flush=True)
