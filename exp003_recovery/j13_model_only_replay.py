#!/usr/bin/env python3
"""EXP-003 Clausura 2025 J13 model-only replay.

Temporal contract
-----------------
- Target slate: Clausura 2025 Jornada 13, 28-30 March 2025.
- One global model freeze at 2025-03-28 00:00. No J13 result is eligible.
- J1-J12 are historical and may enter training.
- External price/context snapshot cutoff: 2025-03-28 18:59 America/Mexico_City.
...
MODEL_CUTOFF = datetime(2025, 3, 28, 0, 0, 0)
CONTEXT_CUTOFF = "2025-03-28T18:59:00-06:00"
EXPECTED_TRAINING_ROWS = 955

FIXTURES = [
    ("J13-01", "2025-03-28", "Club Necaxa", "Gallos Blancos"),
    ("J13-02", "2025-03-28", "Mazatlán FC", "Atlas Guadalajara"),
    ("J13-03", "2025-03-29", "FC Juárez", "Puebla FC"),
    ("J13-04", "2025-03-29", "Deportivo Toluca", "CF Pachuca"),
    ("J13-05", "2025-03-29", "CF América", "UANL Tigres"),
    ("J13-06", "2025-03-29", "Deportivo Guadalajara", "Cruz Azul"),
    ("J13-07", "2025-03-29", "CF Monterrey", "Club Tijuana"),
    ("J13-08", "2025-03-30", "Santos Laguna", "Atlético San Luis"),
    ("J13-09", "2025-03-30", "Club León", "Pumas UNAM"),
]
...
def main():
    ...
    if len(tr) != EXPECTED_TRAINING_ROWS:
        raise SystemExit(f"J13 chronology drift: expected {EXPECTED_TRAINING_ROWS}, got {len(tr)}")
    ...
    out = {
        "track": "HISTORICAL_J13_MODEL_ONLY_PRE_CONTEXT",
        ...
        "warning": "MODEL_ONLY pre-context. Human researcher is outcome-exposed for J13; selection must remain deterministic model+price only."
    }
    print("J13_MODEL_ONLY_OUTPUT", json.dumps(out, ensure_ascii=False, sort_keys=True), flush=True)
