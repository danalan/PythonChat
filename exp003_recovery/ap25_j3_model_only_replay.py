#!/usr/bin/env python3
"""Apertura 2025 J3 model-only replay under frozen V0.1R + calibration shadow.

Integrity note: researcher became outcome-exposed to at least one J3 result during
schedule research on 2026-09-14. Therefore J3 is permanently NO-PROMOTION.
No J3 outcome is used as an input. Training includes only matches prior to
2025-07-25, including completed Apertura 2025 J1 and J2 fixtures.
"""
...
MODEL_CUTOFF = datetime(2025, 7, 25, 0, 0, 0)
CAL_K = 0.5620833333
...
FIXTURES = [
    ("AP25J3-01","2025-07-25","Gallos Blancos","Pumas UNAM"),
    ("AP25J3-02","2025-07-25","Puebla FC","Santos Laguna"),
    ("AP25J3-03","2025-07-25","Club Tijuana","FC Juárez"),
    ("AP25J3-04","2025-07-26","CF Pachuca","Mazatlán FC"),
    ("AP25J3-05","2025-07-26","Deportivo Guadalajara","Atlético San Luis"),
    ("AP25J3-06","2025-07-26","Cruz Azul","Club León"),
    ("AP25J3-07","2025-07-26","Deportivo Toluca","UANL Tigres"),
    ("AP25J3-08","2025-07-26","CF Monterrey","Atlas Guadalajara"),
    ("AP25J3-09","2025-07-26","Club Necaxa","CF América"),
]
...
def main():
    ...
    tr=sorted(hist + J1_FROZEN + J2_FROZEN, key=lambda m:(m.date,m.home,m.away,m.hg,m.ag))
    if len(tr)!=1033:
        raise SystemExit(f"J3 training count drift: expected 1033, got {len(tr)}")
    if any(m.date >= MODEL_CUTOFF for m in tr):
        raise SystemExit("J3 leakage detected")
    ...
    out={
        ...
        "model_cutoff":"2025-07-25T00:00:00-06:00",
        "training_count":len(tr),
        "training_includes":"Apertura 2025 J1 + J2 completed fixtures",
        "integrity_class":"DETERMINISTIC_REPLAY_RESEARCHER_OUTCOME_EXPOSED",
        "promotion_eligibility":False,
        "fixtures":rows,
        "notes":[
            "No J3 result is an input.",
            "Raw V0.1R remains canonical for selection; calibration remains shadow-only.",
            "J3 cannot promote model/context/calibration changes because researcher saw at least one J3 outcome during schedule research.",
        ],
    }
    print("AP25_J3_MODEL_ONLY", json.dumps(out, ensure_ascii=False, sort_keys=True), flush=True)
