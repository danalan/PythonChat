#!/usr/bin/env python3
"""Apertura 2025 J11 mechanical V2.1 selector + V2.2 fractional-Kelly shadows."""
from __future__ import annotations
import json
from pathlib import Path
ROOT=Path(__file__).resolve().parent
MODEL_OUT=ROOT/'ap25_j11_model_output.txt'
PRICE_SNAPSHOT=ROOT/'ap25_j11_price_snapshot_pre_cutoff.json'
BUDGET=100.0
CANONICAL_OPENING=447.3012159211471
SHADOW_OPENINGS={'FK15':478.8569655685746,'FK20':475.298173774325,'FK25':471.4882997472078}
...
def load_model():
    text=MODEL_OUT.read_text(encoding='utf-8').strip(); prefix='AP25_J11_MODEL_ONLY '
    if not text.startswith(prefix): raise SystemExit('Unexpected J11 model output format')
    obj=json.loads(text[len(prefix):])
    if obj['training_count']!=1106: raise SystemExit('J11 training-count drift')
    if obj['jornada']!='J11': raise SystemExit('Wrong model output')
    return obj
...
def main():
    ...
    out={'experiment':'EXP-003','tournament':'Apertura 2025','jornada':'J11',
         'integrity_class':'MECHANICAL_REPLAY_EXTERNAL_PACKET_VISIBLE_J11_OUTCOME_EXPOSURE','promotion_eligibility':False,
         ...
         'notes':['No J11 outcome is used in calculations.','Only Juarez-Leon has an accepted complete pre-kickoff 1X2 row.',
                  'Supplied packet table/form/splits were rejected because they contradict canonical frozen history.',
                  'J11 is permanently NO-PROMOTION because visible source titles in the external packet exposed J11 outcomes.']}
    print('AP25_J11_SELECTION '+json.dumps(out,ensure_ascii=False,sort_keys=True),flush=True)
