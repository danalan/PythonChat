...
LO=30/169
HI=1-30/169
# Frozen aggregate-error penalties recovered from legacy J6 audit universe.
BIAS={
...
ERR_ENVELOPE=0.0030  # slightly above worst J6 target discrepancy observed in recovery
...
def probs(mat):
    ...
    return {
      'RESULT_HOME':home,'RESULT_AWAY':away,
      'DOUBLE_1X':home+draw,'DOUBLE_X2':away+draw,'DOUBLE_CHANCE|12':home+away,
      'TEAM_TOTALS|HOME_OVER_0.5':1-h0,
      'TEAM_TOTALS|HOME_UNDER_2.5':h_under25,
      'MATCH_TOTALS|OVER_1.5':over15,
    }
...
for fid,date,home,away in p.J8_FIXTURES:
    mat=p.ensemble_matrix(mods,home,away); q=probs(mat)
    for ident,pr in q.items():
        score=pr-BIAS[ident]
        eligible=(pr>=0.60 and LO<=pr<=HI)
        # robust only if probability remains inside/outside same side of each gate under error envelope
...
# Selection: distinct matches, max 2 same broad family (DC/TEAM/MATCH/RESULT)
def fam(x):
    z=x['identity']
