...
def probs(mat):
    n=mat.shape[0]
    home=float(sum(mat[i,j] for i in range(n) for j in range(n) if i>j))
    draw=float(sum(mat[i,i] for i in range(n)))
    away=float(1-home-draw)
    h0=float(sum(mat[0,j] for j in range(n))); h_under25=float(sum(mat[i,j] for i in range(3) for j in range(n)))
    over15=float(sum(mat[i,j] for i in range(n) for j in range(n) if i+j>=2))
...
for fid,date,home,away in p.J8_FIXTURES:
    ...
    for ident,pr in q.items():
        score=float(pr-BIAS[ident])
        eligible=bool(pr>=0.60 and LO<=pr<=HI)
        dist=float(min(abs(pr-0.60),abs(pr-LO),abs(pr-HI)))
        robust=bool(dist>ERR_ENVELOPE)
        rows.append({'fixture_id':fid,'date':date,'home':home,'away':away,'identity':ident,'p':float(pr),'bias_abs':float(BIAS[ident]),'score':score,'eligible':eligible,'gate_distance':dist,'robust_gate':robust})
...
print('J8_ANY_GATE_AMBIGUITY_TOP20',bool(any(not r['robust_gate'] for r in pool[:20])))
