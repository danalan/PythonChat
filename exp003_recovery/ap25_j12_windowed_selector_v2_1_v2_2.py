#!/usr/bin/env python3
"""Apertura 2025 J12 mechanical V2.1 selector + V2.2 fractional-Kelly shadows."""
from __future__ import annotations
import json
from pathlib import Path
ROOT=Path(__file__).resolve().parent
MODEL_OUT=ROOT/'ap25_j12_model_output.txt'
PRICE_SNAPSHOT=ROOT/'ap25_j12_price_snapshot_pre_cutoff.json'
BUDGET=100.0
CANONICAL_OPENING=437.3012159211471
SHADOW_OPENINGS={'FK15':475.09201602517317,'FK20':470.3155484227055,'FK25':465.30994244189486}
...
def load_model():
    text=MODEL_OUT.read_text(encoding='utf-8').strip(); prefix='AP25_J12_MODEL_ONLY '
    if not text.startswith(prefix): raise SystemExit('Unexpected J12 model output format')
    obj=json.loads(text[len(prefix):])
    if obj['training_count']!=1115: raise SystemExit('J12 training-count drift')
    if obj['jornada']!='J12': raise SystemExit('Wrong model output')
    return obj
...
def main():
    ...
    for f in snap['fixtures']:
        ...
        for market,decimal in f['prices'].items():
            ...
            rows.append({'fixture_id':fid,**fixture_meta[fid],'market':market,'decimal':o,'p_raw':p,'break_even':1.0/o,
                         'model_ev':ev,'kelly_score':k,'bookmaker':f['bookmaker'],'source':f['source'],'source_url':f['source_url']})
    best=[]
    ...
    out={'experiment':'EXP-003','tournament':'Apertura 2025','jornada':'J12',
         'integrity_class':'MECHANICAL_REPLAY_EXTERNAL_PACKET_VISIBLE_J12_OUTCOME_EXPOSURE_BEFORE_SELECTION_FREEZE','promotion_eligibility':False,
         ...
         'notes':['No J12 outcome is used in calculations.','Only two fixtures have accepted complete pre-kickoff 1X2 rows after strict internal audit.',
                  'Canonical V2.1 uses Kelly for ranking/gating and fixed stake bands.','V2.2 shadows inherit canonical qualifying singles and fixed secondary parlays; only single sizing changes.']}
    print('AP25_J12_SELECTION '+json.dumps(out,ensure_ascii=False,sort_keys=True),flush=True)
