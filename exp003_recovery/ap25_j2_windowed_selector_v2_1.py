#!/usr/bin/env python3
"""Apertura 2025 J2 mechanical price-aware selector under frozen Execution Policy V2.1.

J2 researcher outcome exposure occurred before the final price snapshot. Therefore:
...
- J2 is permanently NO-PROMOTION regardless of settlement.

The J1 staking architecture is preserved:
- positive EV required;
- within each fixture keep highest full-Kelly fraction as risk-adjusted ranking score;
- primary single gate Kelly >= .05;
- fixed stakes: >=.15 -> 20 MXN, >=.08 -> 15 MXN, >=.05 -> 10 MXN;
- one 5 MXN diagnostic two-leg parlay per window when >=2 primary singles exist;
- weekly exposure may not exceed 100 MXN and early returns cannot replenish it.
"""
from __future__ import annotations
import json
from pathlib import Path
import numpy as np
import ap25_j2_model_only_replay as mdl

BUDGET=100.0
OPENING_BANKROLL=528.2012159211471
SNAP=Path(__file__).with_name('ap25_j2_price_snapshot_pre_cutoff.json')
WINDOW_ORDER={"WEDNESDAY":0,"FRIDAY":1,"SATURDAY":2,"SUNDAY":3}
...
def main():
    ...
    tr=sorted(hist+mdl.J1_FROZEN,key=lambda m:(m.date,m.home,m.away,m.hg,m.ag))
    if len(tr)!=1024: raise SystemExit(f"J2 training count drift: expected 1024, got {len(tr)}")
    ...
    if total>BUDGET:
        raise SystemExit(f"Singles alone exceed frozen weekly budget: {total}")
    parlays=[]
    for window in ("WEDNESDAY","FRIDAY","SATURDAY","SUNDAY"):
        ...
        if len(wr)>=2 and total+5.0<=BUDGET:
            ...
            parlays.append({"window":window,"vehicle":"PARLAY_2_SECONDARY","stake_mxn_simulated":stake,
            ...
            total+=stake
    out={
        "experiment":"EXP-003","tournament":"Apertura 2025","jornada":"J2",
        ...
        "notes":[
            ...
            "Human context cannot alter J2 because researcher outcome exposure occurred before final freeze.",
            "Only explicitly archived pre-kickoff/historical opening prices in the frozen snapshot are eligible.",
            "Returns from early windows cannot replenish the MXN100 risk budget."
        ]
    }
    print("AP25_J2_WINDOWED_SELECTION",json.dumps(out,ensure_ascii=False,sort_keys=True),flush=True)
