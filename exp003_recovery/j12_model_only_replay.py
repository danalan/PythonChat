#!/usr/bin/env python3
"""EXP-003 Clausura 2025 J12 model-only replay.

Temporal contract
-----------------
- Target slate: Clausura 2025 Jornada 12, 14-16 March 2025.
- One global model freeze at 2025-03-14 00:00. No J12 result is eligible.
- Context is NOT consumed by this script. External context/price snapshots must
  be public no later than 2025-03-14 18:59 America/Mexico_City.
- J11 is now historical and may enter training; J12 remains sealed target data.
- Pricing is a separate layer governed by ECONOMIC_TRACK_V1.
"""
...
MODEL_CUTOFF = datetime(2025, 3, 14, 0, 0, 0)
CONTEXT_CUTOFF = "2025-03-14T18:59:00-06:00"
EXPECTED_TRAINING_ROWS = 946

FIXTURES = [
    ("J12-01", "2025-03-14", "Gallos Blancos", "Mazatlán FC"),
    ("J12-02", "2025-03-14", "Club Necaxa", "Club León"),
    ("J12-03", "2025-03-14", "Puebla FC", "Deportivo Toluca"),
    ("J12-04", "2025-03-15", "UANL Tigres", "Santos Laguna"),
    ("J12-05", "2025-03-15", "Cruz Azul", "Atlético San Luis"),
    ("J12-06", "2025-03-15", "FC Juárez", "Deportivo Guadalajara"),
    ("J12-07", "2025-03-15", "Atlas Guadalajara", "CF América"),
    ("J12-08", "2025-03-16", "Pumas UNAM", "CF Monterrey"),
    ("J12-09", "2025-03-16", "CF Pachuca", "Club Tijuana"),
]
...
def main():
    ...
    if len(tr) != EXPECTED_TRAINING_ROWS:
        raise SystemExit(
            f"J12 chronology drift: expected {EXPECTED_TRAINING_ROWS} training rows, got {len(tr)}"
        )
    ...
    out = {
        "track": "HISTORICAL_J12_MODEL_ONLY_PRE_CONTEXT",
        ...
        "warning": (
            "MODEL_ONLY pre-context output. J12 outcomes remain sealed. "
            "Price/context layers must obey cutoff and may not rewrite p_model."
        ),
    }
    print("J12_MODEL_ONLY_OUTPUT", json.dumps(out, ensure_ascii=False, sort_keys=True), flush=True)
